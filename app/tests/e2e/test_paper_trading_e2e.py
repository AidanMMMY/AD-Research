...
def test_paper_trading_full_buy_sell_lifecycle(db_session, crypto_universe):
    ...
    account = svc.create_account("E2E", Decimal("10000"), user_id=1)
    ...
    # SELL 0.1 BTC @ 120
    with patch.object(PaperTradingService, "_get_provider_for_code", return_value=_provider(120.0)):
...
def test_paper_trading_open_position_unrealized_pnl(db_session, crypto_universe):
    ...
    with patch.object(PaperTradingService, "_get_provider_for_code", return_value=_provider(150.0)):
        summary = svc.get_pnl_summary(account.id)
...
def test_paper_trading_insufficient_cash_raises(db_session, crypto_universe):
    ...
    svc = PaperTradingService(db_session)
    account = svc.create_account("Small", Decimal("10"), user_id=1)
    with patch.object(PaperTradingService, "_get_provider_for_code", return_value=_provider(100.0)):
...
def test_paper_trading_insufficient_position_raises(db_session, crypto_universe):
    ...
    svc = PaperTradingService(db_session)
    account = svc.create_account("NoPos", Decimal("10000"), user_id=1)
    with patch.object(PaperTradingService, "_get_provider_for_code", return_value=_provider(100.0)):
...
def test_paper_trading_unknown_instrument_raises(db_session):
    ...
    svc = PaperTradingService(db_session)
    account = svc.create_account("Ghost", Decimal("10000"), user_id=1)
    with patch.object(PaperTradingService, "_get_provider_for_code", return_value=_provider(100.0)):
...
def test_paper_trading_archive_excludes_account(db_session, crypto_universe):
    ...
    svc = PaperTradingService(db_session)
    a1 = svc.create_account("Active", Decimal("1000"), user_id=1)
    a2 = svc.create_account("ToArchive", Decimal("1000"), user_id=1)
...
def test_paper_trading_position_aggregation_vwap(db_session, crypto_universe):
    ...
    svc = PaperTradingService(db_session)
    account = svc.create_account("VWAP", Decimal("100000"), user_id=1)

    with patch.object(PaperTradingService, "_get_provider_for_code", return_value=_provider(100.0)):
        svc.place_order(account.id, "BTC.US", "BUY", Decimal("1"))
    with patch.object(PaperTradingService, "_get_provider_for_code", return_value=_provider(200.0)):
        svc.place_order(account.id, "BTC.US", "BUY", Decimal("1"))
