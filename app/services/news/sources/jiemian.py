...
JIEMIAN_RSS_URL = "https://a.jiemian.com/index.php?m=article&a=rss"


class JiemianCrawler(BaseCrawler):
    """Crawl Jiemian RSS feed."""
    ...
    async def crawl(self) -> list[RawArticle]:
        if not await is_robots_allowed(JIEMIAN_RSS_URL):
            logger.warning("%s blocked by robots.txt", self.source_name)
            return []
        response = await self.fetch(JIEMIAN_RSS_URL)
    ...
    async def run(self, db: Any) -> int:
        ...
        for raw in raw_articles:
            try:
                ...
                logger.warning("Jiemian normalize failed: %s", exc)
        db.commit()
        return inserted
