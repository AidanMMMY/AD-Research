...
HUXIU_RSS_URL = "https://rss.huxiu.com/"


class HuxiuCrawler(BaseCrawler):
    """Crawl Huxiu RSS feed."""
    ...
    async def crawl(self) -> list[RawArticle]:
        if not await is_robots_allowed(HUXIU_RSS_URL):
            logger.warning("%s blocked by robots.txt", self.source_name)
            return []
        response = await self.fetch(HUXIU_RSS_URL)
    ...
    async def run(self, db: Any) -> int:
        ...
        for raw in raw_articles:
            try:
                ...
                logger.warning("Huxiu normalize failed: %s", exc)
        db.commit()
        return inserted
