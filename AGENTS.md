...
- **后台任务**：APScheduler（Asia/Shanghai，进程内调度）+ Celery 5（Redis broker，队列：`indicator`、`cninfo`/`celery`）
...
scripts/                # 运维脚本（回填、审计、备份、部署检查等，约 80 个）
...
python scripts/seed_users.py      # 初始化 admin 等账号（幂等）
...
- 测试按模块组织：`test_api/`、`services/`、`strategies/`、`test_data/`、`e2e/` 及根级 `test_*.py`
...
- Celery app 在 `app/core/celery_app.py`；worker 按队列拆分：`indicator`（指标计算）与 `celery,cninfo`（巨潮公告/PDF）
