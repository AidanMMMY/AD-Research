...
| **AI/LLM** | Anthropic Claude (Haiku 默认 / Sonnet 可选), Redis缓存 |
...
┌──────────────────────────────────────────────────────┐
│                    存储层                              │
│  PostgreSQL: etf_info, etf_daily_bar, etf_indicator,  │
...
| **Anthropic** | 按量付费 | AI研报/情绪/对话 | [console.anthropic.com](https://console.anthropic.com/) |

**月度成本预估**: 数据 $0 + LLM ~$5-15 (Claude Haiku)
...
| 文件 | 改动 |
|------|------|
| `web/src/types/etf.ts` | 新增 instrument_type/sector/market_cap 字段 |
| `web/src/pages/ETFList/index.tsx` | 新增"类型"下拉筛选(ETF/个股) + Tag标签 + 规模列支持美股B/M/T显示 |
...
├── anthropic_provider.py            # Claude 实现（Haiku + Sonnet）
...
app/api/v1/research.py               # 8个AI REST端点
...
| 场景 | 模型 | 成本 (每1M token) | 用途 |
|------|------|-------------------|------|
| **默认** | Claude Haiku 3.5 | $0.25 输入 / $1.25 输出 | 研报生成、情绪分类、日常对话 |
| **复杂分析** | Claude Sonnet 4 | $3 输入 / $15 输出 | 深度研报、财报全面解读 |
...
```python
# config.py
anthropic_model: str = "claude-haiku-3-5-20241022"  # 默认低成本
llm_monthly_budget: float = 15.0                      # 月度预算上限
...
/login              # 登录
/dashboard          # 首页看板
/etfs               # ETF/个股列表（支持类型筛选）
/etfs/:code         # 详情页（K线 + 指标 + AI分析Tab）
/screen             # 全市场筛选器
/pools              # 标的池管理
/pools/:id          # 池详情
/scores             # 评分排名
...
A: 系统默认使用Claude Haiku ($0.25/MTok)，加上Redis缓存，月度$5-15即可。可在 `app/config.py` 设 `llm_monthly_budget` 控制上限。如果未设 `ANTHROPIC_API_KEY`，AI功能会自动跳过（不会crash）。
