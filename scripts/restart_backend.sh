...
docker rm -f alloyresearch-backend 2>/dev/null || true

docker run -d \
  --name alloyresearch-backend \
...
for i in $(seq 1 15); do
  if docker logs alloyresearch-backend 2>&1 | grep -q "Application startup complete"; then
