#!/usr/bin/env node
/**
 * Phase 7 ECS production validation script.
 *
 * Usage:
 *   cd web && BASE_URL=http://47.239.13.111:8000 node scripts/visual-regression-ecs.mjs
 *
 * Unlike the local version, this script does NOT start a Vite dev server;
 * it talks directly to the production Nginx + backend endpoint.
 */
...
async function main() {
    ...
    for (const viewport of VIEWPORTS) {
      for (const themeDef of THEMES) {
        ...
        for (const pageDef of PAGES) {
          ...
          // Throttle requests to avoid tripping ECS/nginx rate limits.
          await page.waitForTimeout(2500);
        }

        await context.close();
      }
      // Cool down between viewports to avoid ECS/nginx rate-limit resets.
  ...
  console.log("\n=== Phase 7 ECS Validation Summary ===");
