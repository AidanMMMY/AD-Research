...
const entries: TermEntry[] = [
  ...
  },
  {
    ...
    fullDesc: '趋势强度维度综合 RSI 和价格在均线之上的位置，判断当前是否处于较强上升趋势。',
    formula: '综合 rsi14、ma_position 等指标后做百分位排名',
  ...
  },
  {
    ...
    fullDesc: '波动率反映资产价格波动的剧烈程度。20 日波动率用最近 20 个交易日数据计算，并年化处理。',
    formula: 'std(近20日收益率) × √252 × 100%',
    interpretation: '数值越高，价格波动越大，风险也越高。适合用于仓位控制和止损设置。',
  ...
  },
  {
    ...
    fullDesc: '布林带通过价格的标准差构建波动通道。价格触及上轨可能超买，触及下轨可能超卖。',
    formula: '中轨 = MA20；上轨 = MA20 + 2 × std20；下轨 = MA20 - 2 × std20',
    interpretation: '通道收窄预示波动即将放大；价格突破上轨或下轨可能意味着趋势延续或反转。',
    relatedPageType: 'etf_detail',
  },
  {
    key: 'macd',
  ...
  },
  {
    key: 'risk_parity',
    title: '风险平价',
    shortDesc: '按波动率倒数加权，低波动品种权重更高。',
    fullDesc: '风险平价目标不是让资金平均分配，而是让每只 ETF 对组合整体风险的贡献尽量均衡。低波动品种因此获得更高权重。',
    formula: '某只权重 ∝ 1 / 该只波动率',
    interpretation: '适合希望降低组合波动、让风险更分散的投资者。',
    example: 'ETF A 波动 5%，ETF B 波动 20%，风险平价下 A 的权重约为 B 的 4 倍。',
    relatedPageType: 'pool_detail',
  },
  {
    key: 'rebalance',
    title: '再平衡',
    shortDesc: '当实际权重偏离目标权重超过阈值时，调整持仓使其回归目标。',
    fullDesc: '由于市场价格波动，实际市值权重会偏离目标权重。再平衡通过买入低配品种、卖出高配品种，让组合回到目标配置。',
    interpretation: '再平衡能纪律性地“高抛低吸”，但过于频繁会增加交易成本。',
    relatedPageType: 'pool_detail',
  },
  {
    key: 'target_weight',
  ...
  },
  {
    ...
    title: '池整体表现',
    shortDesc: '标的池按当前权重加权计算后的收益、夏普、回撤等综合表现。',
    fullDesc: '池整体表现把池内各只 ETF 的权重和收益结合起来，计算加权后的 1 月/3 月收益、夏普比率和最大回撤，反映整个组合的历史表现。',
    interpretation: '组合表现不仅取决于单只 ETF，还取决于权重配置和相关性结构。',
    relatedPageType: 'pool_detail',
  },
  {
    key: 'strategy_type',
