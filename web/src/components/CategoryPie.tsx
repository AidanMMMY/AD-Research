...
export default function CategoryPie({ data, mode = 'count' }: CategoryPieProps) {
  ...
  // Resolve all CSS-variable colors once per render. Uses the single
  // chart-color resolver (chartColors.ts) with bare token names; explicit
  // fallbacks mirror the dark-theme defaults in theme.css so SSR / no-DOM
  ...
  const bgElevated = useMemo(
    () => resolveChartColors(['--bg-elevated'], ['#161B22']),
    [],
  );
  const borderDefault = useMemo(
    () => resolveChartColors(['--border-default'], ['#30363D']),
    [],
  );
  const textPrimary = useMemo(
    () => resolveChartColors(['--text-primary'], ['#E6EDF3']),
    [],
  );
  const textSecondary = useMemo(
    () => resolveChartColors(['--text-secondary'], ['#A0A0A0']),
    [],
  );
