...
## 执行记录（2026-07-30 03:35-03:50 +08）

1. **f9a255a** 主集成 commit：CI + Deploy 全绿（3m38s）。ECS scheduler 确认 4 个
   `news_zhb_{a-d}_60m` job 注册。
2. **冒烟 batch a 发现 iplaysoft 解析失败**：其缓存插件输出 `<!--Cached 1785347478--->`，
   `--->` 违反 XML 注释规范（闭合必须恰好 `-->`），严格 `ET.fromstring` 整源丢弃。
3. **48605ee** rss_common 容错恢复 commit：严格解析失败后重试修复版
   （剥非法控制字符 + 折叠 `-{3,}>` → `-->`），全站 652 源受益；
   3 条恢复测试，本地 578 passed，CI + Deploy 全绿。
4. **ECS 全量冒烟（4 批）**：a=95 篇/10 源，b=100/10，c=90/10，d=80/8，
   **合计 365 篇 / 38 源全活**（iplaysoft 已恢复，单源 30 条）。
5. **wechat3/zhx "无 etl 记录" 虚惊闭环**：实为调度器冻结窗口所致，restart 后
   02:29-02:40 +08 全部 7 个 job 成功执行；zhx 300 篇/30 源、wechat3 22/22 源全有文
   （源名是 `wechat_{slug}` 不是 `wechat3_*`，SQL 用错前缀才显示 0）。
6. **目标达成**：公众号 22 + 播客 40 + 博客 38 = **100 个中文圈独立思考资讯源**。
   投资界 wewe-rss 重加（等用户文章链接）后将成为第 101 个。
