...
**日期**：2026-07-19
**触发**：解决 `app/services/sector_rotation_service.py` 文件尾部的 Phase 3 TODO
**作者**：总管 Agent（用户授权 A 股/AKShare 路径）

> 最后核实更新：2026-07-21（调度已由「每周一 09:30」改为「每日 20:00」，详见决策表）
...
| 调度 | ~~每周一 09:30~~ → **每日 (mon-fri) 20:00** Asia/Shanghai（2026-07-20 起调整） | 数据变化慢；改为每日晚间是因为 7/20 09:30 盘前跑成功但 max(trade_date) 仍停留在 7/16 —— 申万指数当日数据晚间才发布，每日 20:00 跑保证当日数据当夜落库 |
...
- `app/core/celery_app.py` — include `app.tasks.sw_industry`,
  route `industry` 队列
- `app/core/scheduler.py` — cron 每日 (mon-fri) 20:00 Asia/Shanghai（最初为每周一 09:30，7/20 起调整，原因见决策表）
...
- TODO 位置：`app/services/sector_rotation_service.py` 文件尾部注释块（原 :808，现约 :926，已注释关闭）
